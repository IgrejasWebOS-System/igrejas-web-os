      ...
      <div className="bg-neutral-900 border border-neutral-800 rounded-2xl p-6 shadow-xl flex flex-col lg:flex-row lg:items-center justify-between gap-6">
        ...
        <div className="flex flex-col sm:flex-row gap-3 items-center w-full lg:w-auto">
          <form action={handleAdd} className="flex flex-col sm:flex-row gap-3 items-center w-full sm:w-auto">
              <input 
                type="text" name="name" placeholder="Ex: CASADO(A)" value={newName} onChange={(e) => setNewName(e.target.value)} 
                className="w-full sm:w-56 bg-neutral-950 border border-neutral-800 rounded-xl py-3 px-4 text-sm text-white focus:outline-none focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500/50 transition-all font-medium uppercase placeholder:normal-case placeholder:text-neutral-700" required 
              />
              <button type="submit" disabled={isSubmitting || !newName.trim()} className="w-full sm:w-auto bg-emerald-600 hover:bg-emerald-500 disabled:opacity-50 text-white px-6 py-3 rounded-xl font-bold transition-colors flex items-center justify-center gap-2 shadow-lg shadow-emerald-900/20 uppercase whitespace-nowrap">
