...
import { ArrowLeft, Loader2, Trash2, Network, AlertCircle } from "lucide-react";
    ...
    <div className="max-w-4xl mx-auto space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-10">
      ...
      <div className="bg-neutral-900 border border-neutral-800 rounded-2xl p-6 shadow-xl flex flex-col lg:flex-row lg:items-center justify-between gap-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-xl font-black text-white tracking-tight uppercase">DEPARTAMENTOS</h1>
          </div>
        </div>

        <div className="flex flex-col sm:flex-row gap-3 items-center w-full lg:w-auto">
          <form action={handleAdd} className="flex flex-col sm:flex-row gap-3 items-center w-full sm:w-auto">
              <input 
                  type="text" 
                  name="name" 
                  ...
                  className="w-full sm:w-56 bg-neutral-950 border border-neutral-800 rounded-xl py-3 px-4 text-sm text-white focus:outline-none focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500/50 transition-all font-medium uppercase placeholder:normal-case placeholder:text-neutral-700" 
                  required 
              />
              <button 
                type="submit" 
                disabled={isSubmitting || !newName.trim()} 
                className="w-full sm:w-auto bg-emerald-600 hover:bg-emerald-500 disabled:opacity-50 text-white px-6 py-3 rounded-xl font-bold transition-colors flex items-center justify-center gap-2 shadow-lg shadow-emerald-900/20 uppercase whitespace-nowrap"
              >
          ...
          <Link 
            ...
            className="w-full sm:w-auto flex items-center justify-center gap-2 p-3 px-6 bg-neutral-900 border border-neutral-800 rounded-xl hover:bg-neutral-800 hover:text-white text-neutral-400 transition-colors text-sm font-bold uppercase whitespace-nowrap"
          >
        ...
        {loading ? ( 
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left text-sm">
                  <thead className="bg-neutral-950/50 text-sm font-bold text-neutral-400 border-b border-neutral-800 uppercase tracking-wide">
                    <tr>
                      <th className="px-6 py-4">Descrição do Registro</th>
                          ...
                          items.map((item) => (
                              <tr key={item.id} className="hover:bg-neutral-800/80 transition-colors group">
                                  ...
                                  <td className="px-6 py-4 text-right w-20">
                                      <button onClick={() => handleDelete(item.id)} className="p-2 text-neutral-500 hover:text-red-500 hover:bg-neutral-800 rounded-lg transition-colors inline-flex items-center justify-center">
